...
describe("XO config", () => {
  ...
  it("should have TypeScript plugin enabled in TS overrides", () => {
    const tsOverride = config.overrides.find(
      (o) => o.files.includes("**/*.ts") || o.files.includes("**/*.tsx")
    );
  ...
  it("should have Prettier plugin enabled in JS/TS overrides", () => {
    ...
    relevantOverrides.forEach((o) => {
      if (o.extends) {
        const hasPrettier = o.extends.some((ext) =>
          ext.toLowerCase().includes("prettier")
        );
  ...
  it("should include Stylistic plugin rules", () => {
    const stylisticRules = Object.keys(config.rules).filter((rule) =>
